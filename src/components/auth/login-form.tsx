...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const target =
        data.user.role === "ADMIN" ? "/admin/dashboard" : "/seller/dashboard";
  ...
  return (
    <div className="min-h-screen w-full flex">
      ...
      <div
        className="hidden lg:flex lg:w-[52%] relative overflow-hidden flex-col items-center justify-center px-16"
        style={{
          background:
            "linear-gradient(135deg, oklch(0.22 0.06 270) 0%, oklch(0.14 0.04 264) 50%, oklch(0.18 0.08 240) 100%)",
        }}
      >
        {/* Decorative glowing orbs */}
        <div
          className="absolute top-[-10%] left-[-10%] w-[520px] h-[520px] rounded-full opacity-20 blur-3xl animate-float"
          style={{
            background:
              "radial-gradient(circle, oklch(0.66 0.20 270), transparent 70%)",
          }}
        />
        <div
          className="absolute bottom-[-10%] right-[-10%] w-[420px] h-[420px] rounded-full opacity-15 blur-3xl animate-float"
          style={{
            background:
              "radial-gradient(circle, oklch(0.65 0.17 215), transparent 70%)",
            animationDelay: "3s",
          }}
        />
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] rounded-full opacity-10 blur-2xl"
          style={{
            background:
              "radial-gradient(circle, oklch(0.68 0.19 145), transparent 70%)",
          }}
        />
        ...
        <div className="relative z-10 max-w-md text-center space-y-8">
          ...
          <p
            ...
            style={{ color: "oklch(0.80 0.06 270)" }}
          >
          ...
          >
            Streamline your pharmaceutical chemical supply chain with precision
            inventory tracking, real-time quotations, and order management.
          </p>
      ...
      {/* ── Right panel: form ── */}
      <div className="relative flex-1 flex items-center justify-center px-6 py-12 bg-background">
        <div className="absolute right-4 top-4 z-20">
          <ThemeToggle className="h-9 rounded-xl border border-border/70 bg-background/80 px-3 text-muted-foreground shadow-sm backdrop-blur hover:text-foreground hover:bg-background" />
        </div>
          ...
          <div className="rounded-2xl border border-border bg-card shadow-lg p-7 space-y-5">
            ...
            >
              ...
              <button
                ...
                style={{
                  background:
                    "linear-gradient(135deg, var(--primary) 0%, oklch(0.55 0.22 280) 100%)",
